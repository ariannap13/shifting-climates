...
for year in range(2022, 2024):
    for month in range(1, 13):
        ...
        date_ranges = generate_date_ranges(
            start_date_str=start, total_days=365
        )


        load_dotenv()

        start_query = start.replace("-", "")
                            ...
                            "and": [
                                    {
                                        ...
                                        "field_values": ["SHORT", "MID", "LONG", "EXTRA_LONG"]
                                    },
                                ],
                            },
                "max_count": 100,
        ...
        # save json
        # create a the baseline folder if it does not exist
        if not os.path.exists("./data/search/baseline"):
            os.makedirs("./data/search/baseline")
        
        

        save_json(
            path=Path(f"./data/search/baseline/baseline2_{year}_{month}.json"),
            container=videos,
        )